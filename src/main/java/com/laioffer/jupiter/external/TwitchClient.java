...
public class TwitchClient {
    private static final String TOKEN = "Bearer 3x8fes7xpgo6zkwem01ti34mlo86cv";
    private static final String CLIENT_ID = "w1e6fzwf0xvwhxx8tlemiok9a76n6p";
    ...
    private String buildGameURL(String url, String gameName, int limit) {
        if (gameName.equals("")) {
            return String.format(url, limit); //convert %s to limit number
        } else {
            try {
                gameName = URLEncoder.encode(gameName, "UTF-8"); // space converted to %20
    ...
    private String searchTwitch(String url) throws TwitchException {
        ...
        ResponseHandler<String> responseHandler = response -> {  //input is response,
            ...
            return obj.getJSONArray("data").toString(); // get value for value after "data:"
        };
        try {
    ...
    // Integerate search() and getGameList() together, returns the dedicated game based on the game name.
